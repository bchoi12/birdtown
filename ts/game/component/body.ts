...
type ReadyFn = (body : Body) => boolean;
type InitFn = (body : Body) => void;
type UpdateFn = (body : Body, millis : number) => void;

export type BodyOptions = {
	readyFn? : ReadyFn;
	initFn : InitFn;
	initOptions? : BodyInitOptions;
}
...
export class Body extends ComponentBase implements Component {
	...
	constructor(bodyOptions : BodyOptions) {
		...
		this._readyFn = defined(bodyOptions.readyFn) ? bodyOptions.readyFn : () => { return true; };
		this._initFn = bodyOptions.initFn;

		if (bodyOptions.initOptions) {
			this.initFromOptions(bodyOptions.initOptions);
		}
	}

	initFromOptions(initOptions : BodyInitOptions) : void {
		if (initOptions.pos) { this.setPos(initOptions.pos); }
		if (initOptions.vel) { this.setVel(initOptions.vel); }
		if (initOptions.acc) { this.setAcc(initOptions.acc); }
		if (initOptions.dim) { this.setDim(initOptions.dim); }
	}
	...
	override postPhysics(millis : number) : void {
		...
		if (this.isSource()) {
			...
			// TODO: refine this
			this._pos.lerpSeparate(this._body.position, {x: options.predictionWeight, y: 0});
		}
	}
